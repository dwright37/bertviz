...
def save_embeddings(images_features_labels, save_dir):
    ...
    for name in list(images_features_labels.keys()):
        ...
        [ims, fts, labs, group, sentence] = images_features_labels[name]
        
        # Save sprites and metadata
        if labs is not None:
            metadata_path = os.path.join(save_dir, name + '-metadata.tsv')
            save_metadata(labs, group, sentence, metadata_path)
        if ims is not None:
...
def save_metadata(batch_ys, batch_group, batch_sentence, metadata_path):
    with open(metadata_path,'w') as f:
        f.write("Index\tLabel\tGroup\tSentence\n")
        for index,(label,group, sent) in enumerate(zip(batch_ys, batch_group, batch_sentence)):
            f.write('\t'.join((str(index), str(label), str(group), str(sent))) + '\n')